...
public class CsharpLanguageGenerator : ILanguageGenerator
{
    ...
    public string AttributeDataType(bool isCollection, int rank, string type, bool isGeneric)
    {
        if (isCollection)
            return
                $"{string.Join("", Enumerable.Repeat("List<", rank))}{type}{string.Join("", Enumerable.Repeat(">", rank))}";
    ...
    public string AttributeDataString(AttributeData data)
    {
        ...
        else
        {
            ...
            prop =
                $"\t\tpublic {data.Type}{nullable} {data.Name}{{get;set;}} {(tags.Any() ? "// " + string.Join(",", tags) : string.Empty)}";
        }
...
";
    ...
    {
        if (data.IsCollection) return $"\t\t\t{data.Name} = new {data.Type}();";
        return string.Empty;
    }
    ...
    private string WrappedType(WrapperType data)
    {
        if (data.IsCollectionType)
            return
                $"{string.Join("", Enumerable.Repeat("List<", data.Rank))}{data.WrappedType}{string.Join("", Enumerable.Repeat(">", data.Rank))}";
