...
public class TypescriptGenerator : ILanguageGenerator
{
    ...
    public string AttributeDataType(bool isCollection, int rank, string type, bool isGeneric)
    {
        if (isCollection)
        {
            if (SelectData.ContainsKey(type))
            {
                ...
                return
                    $"{string.Join("", Enumerable.Repeat("Array<", rank))}{unionType}{string.Join("", Enumerable.Repeat(">", rank))}";
            }

            return
                $"{string.Join("", Enumerable.Repeat("Array<", rank))}{type}{string.Join("", Enumerable.Repeat(">", rank))}";
        }
        ...
        if (SelectData.ContainsKey(type)) return string.Join('|', ExpandPossibleTypes(type));
    ...
    public string SimpleTypeString(WrapperType data)
    {
        ...
        var wrappedTypeImport = badTypes.Contains(data.WrappedType)
            ? string.Empty
            : $"import {{{data.WrappedType}}} from \"./{data.WrappedType}.g\"";
        var result =
            $@"
...
";
    ...
    public string EntityString(Entity data)
    {
        ...
        foreach (var d in Dependencies(data)) importBuilder.AppendLine($"import {{{d}}} from \"./{d}.g\"");

        var super = "BaseIfc";
        var newMod = string.Empty;
        if (data.Subs.Any()) super = string.Join(",", data.Subs);
...
{data.Properties(SelectData)}{constructors}{StepParameters(data)}
    ...
    public string FileExtension => "g.ts";
    ...
    public void GenerateManifest(string directory, IEnumerable<string> names,
        Dictionary<string, TypeData> listenerTypeData)
    {
        var importBuilder = new StringBuilder();
        foreach (var name in names) importBuilder.AppendLine($"export * from \"./{name}.g\"");
    ...
    {
        if (data.IsCollection) return $"\t\tthis.{data.Name} = new {data.Type}()";
        return string.Empty;
    }
    ...
    private string WrappedType(WrapperType data)
    {
        if (data.IsCollectionType)
            return
                $"{string.Join("", Enumerable.Repeat("Array<", data.Rank))}{data.WrappedType}{string.Join("", Enumerable.Repeat(">", data.Rank))}";
    ...
    public string EntityConstructorParams(Entity data, bool includeOptional)
    {
        ...
        return string.Join(", ",
            validAttrs.Select(a =>
                $"{a.ParameterName} : {(SelectData.ContainsKey(a.Type) ? string.Join('|', SelectData[a.Type].Values) : a.Type)}"));
    }
...
{data.StepProperties()}
    ...
    private IEnumerable<string> AddRelevantTypes(IEnumerable<AttributeData> attrs)
    {
        ...
        foreach (var a in attrs) result.AddRange(ExpandPossibleTypes(a.type));
